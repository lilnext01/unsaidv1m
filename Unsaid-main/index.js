...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Leia o Codigo Qr acima'))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["555197195835@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`Foi mal man, n consegui converter ${tipe} para figurinha, to baiano dms 😔👍`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('Falha, ocorreu um erro, tente novamente mais tarde. (tlvz eu esteja dormindo) 😎')
							})
				...
				case 'dono':
			     	memein = await kagApi.memeindo()
			     	buffer = await getBuffer('https://i.imgur.com/NG2ORfG.jpg')
				    client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* LILNEXT\n*YOUTUBE:* https://youtube.com/c/unsaid01/\n*WPP:* wa.me/+555197195835\n*INSTA:* https://www.instagram.com/lilnext01/\n\n\n *Aproveite e contemple tudo como é, porque a vida é um sopro*\n*e o sorriso que está aqui hoje, talvez não esteja aqui amanhã...* '})
				    break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'bot':
			     	memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/CXDrhL9.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*É do Lil, tá 🤭*'})
					break
				case 'belle3':
					memein = await kagApi.memeindo()
				...
				case 'canal':
			    	memein = await kagApi.memeindo()
			    	buffer = await getBuffer(`https://i.imgur.com/aKGqccx.jpg `)
			     	client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Canal do LilNext:*\n\n https://youtube.com/c/unsaid01'})
				    break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'teste':
					...
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://upload.wikimedia.org/wikipedia/commons/thumb/4/43/Heraldic_Royal_Crown_%28Common%29.svg/1200px-Heraldic_Royal_Crown_%28Common%29.svg.png`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ola meu dono 🥺️'})
					break
				case 'nsfwloli':
					memein = await kagApi.memeindo()
        ...
        case 'promote':
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse duent @${mentioned[0].split('@')[0]} Acabou de perder o adm pq é baiano dms 😎🛏!`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKKKK acabou de levar ban: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('Responda à um sticker com esse comando')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Consegui converter n man, to com sono foi mal 😔👍')
